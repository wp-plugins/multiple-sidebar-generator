...
class Simple_Page_Sidebars_Widget_Area extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array(
			...
			'description' => __( 'Include all widgets from another widget area', 'multiple-sidebar-generator' )
		);

		// Call the parent constructor.
		$this->WP_Widget( 'area', __( 'Widget Area', 'multiple-sidebar-generator' ), $widget_ops );
	}
	...
	function form( $instance ) {
		...
		?>
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:', 'multiple-sidebar-generator' ); ?></label>
			<input type="text" name="<?php echo $this->get_field_name( 'title' ); ?>" id="<?php echo $this->get_field_id('title'); ?>" value="<?php echo esc_attr( $instance['title'] ); ?>" class="widefat">
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'area_id' ); ?>"><?php _e( 'Area Name:', 'multiple-sidebar-generator' ); ?></label>
