...
class Simple_Page_Sidebars_Admin {
	...
	public static function register_default_sidebar_setting() {
		...
		add_settings_field(
			'simple_page_sidebars_default_sidebar',
			'<label for="multiple-sidebar-generator-default-sidebar">' . __( 'Default Sidebar', 'multiple-sidebar-generator' ) . '</label>',
	...
	public static function default_sidebar_settings_field() {
		...
		?>
		<select name="simple_page_sidebars_default_sidebar" id="multiple-sidebar-generator-default-sidebar">
			<option value=""></option>
			<?php
		...
		<span class="description"><?php _e( 'The sidebar that should be replaced by custom sidebars.', 'multiple-sidebar-generator' ); ?></span>
		<?php
	}
	...
	public static function add_page_sidebar_meta_box( $post_type ) {
		if ( 'page' == $post_type || post_type_supports( $post_type, 'multiple-sidebar-generator' ) ) {
			add_meta_box( 'simplepagesidebarsdiv', __( 'Sidebar', 'multiple-sidebar-generator' ), array( __CLASS__, 'page_sidebar_meta_box' ), $post_type, 'side', 'default' );
		}
	}
	...
	public static function page_sidebar_meta_box( $page ) {
		...
		if ( ! get_option( 'simple_page_sidebars_default_sidebar' )) {
			echo '<div class="multiple-sidebar-generator-page-sidebar-feedback multiple-sidebar-generator-page-sidebar-feedback-error"><p>';
	...
	public static function register_columns( $columns ) {
		$columns['simple-page-sidebar'] = __( 'Sidebar', 'multiple-sidebar-generator' );
		return $columns;
	}
	...
	public static function quick_edit_custom_box( $column, $post_type ) {
		...
		<fieldset class="inline-edit-col-left">
			<div class="inline-edit-col">
				<div class="inline-edit-group" id="multiple-sidebar-generator-page-sidebar-edit-group">
					<label>
						<span class="title"><?php _e( 'Sidebar', 'multiple-sidebar-generator' ); ?></span>
						<select name="simplepagesidebars_page_sidebar_name" id="multiple-sidebar-generator-page-sidebar-name">
		...
		( function( window, $, undefined ) {
			...
			inlineEditPost.edit = function( id ) {
				...
				var postId = inlineEditPost.getId( id ),
					...
					sidebarNameField = $( '#multiple-sidebar-generator-page-sidebar-name' ),
					$nonceField = $( '#multiple-sidebar-generator-page-sidebar-edit-group' ).find( 'input[name="simplepagesidebars_page_sidebar_update_nonce"]' );
		...
		<fieldset class="inline-edit-col-right" style="margin-top: 0">
			<div class="inline-edit-col">
				<label>
					<span class="title"><?php _e( 'Sidebar', 'multiple-sidebar-generator' ); ?></span>
					<select name="simplepagesidebars_page_sidebar_name" id="multiple-sidebar-generator-page-sidebar-name">
						<option value="-1"><?php _e( '&mdash; No Change &mdash;', 'multiple-sidebar-generator' ); ?></option>
						<option value="default"><?php _e( 'Default Sidebar', 'multiple-sidebar-generator' ); ?></option>
						<?php
						foreach ( $sidebars as $sb ) {
	...
	public static function add_sidebar_edit_screen() {
		add_submenu_page( 'admin.php', __( 'Edit Sidebar', 'multiple-sidebar-generator' ), null, 'edit_theme_options', 'multiple-sidebar-generator', array( __CLASS__, 'edit_sidebar_screen' ) );

		add_meta_box( 'simplepagesidebarseditdiv', 'Pages', array( __CLASS__, 'edit_sidebar_pages_meta_box' ), 'admin_page_multiple-sidebar-generator', 'normal', 'default' );
	}
	...
	public static function edit_sidebar_pages_meta_box( $post, $metabox ) {
		...
		include_once( SIMPLE_PAGE_SIDEBARS_DIR . 'admin/includes/class-multiple-sidebar-generator-walker-page-checklist.php' );
	...
	public static function process_sidebar_update() {
		...
		if ( isset( $_POST['simplepagesidebars_sidebar_name'] ) ) {
			...
			if ( empty( $pages ) ) {
				wp_safe_redirect( esc_url_raw( add_query_arg( 'multiple-sidebar-generator-message', 1, admin_url( 'widgets.php' ) ) ) );
				exit;
			}
	...
	public static function widgets_page_messages() {
		...
		if ( isset( $_REQUEST['multiple-sidebar-generator-message'] ) && ! empty( $_REQUEST['multiple-sidebar-generator-message'] ) ) {
			?>
			<div id="message" class="updated">
				<p>
					<?php
					$messages = array(
						1 => __( 'The sidebar you were editing is no longer assigned to any pages and has been removed. Any widgets it contained should be in an "Inactive Widgets" area below.', 'multiple-sidebar-generator' )
					);

					$message_id = $_REQUEST['multiple-sidebar-generator-message'];
	...
	public static function get_sidebar_edit_link( $sidebar, $query_args = array() ) {
		$query_args = wp_parse_args( $query_args, array(
			'page' => 'multiple-sidebar-generator',
	...
	public static function get_empty_default_sidebar_error() {
		return sprintf( __( 'For Page Sidebars Sidebars to work, a default sidebar needs to be selected on the %s', 'multiple-sidebar-generator' ),
			' <a href="' . admin_url( 'options-reading.php' ) . '">' . __( 'Reading settings screen', 'multiple-sidebar-generator' ) . '</a>.'
		);
	}
