<div class="wrap multiple-sidebar-generator-edit-sidebar">
	<div id="icon-tools" class="icon32"><br></div>
	<h2><?php _e( 'Edit Sidebar', 'multiple-sidebar-generator' ); ?></h2>
	...
	if ( ! $wpdb->get_var( $wpdb->prepare( "SELECT post_id FROM $wpdb->postmeta WHERE meta_key='_sidebar_name' AND meta_value=%s", $sidebar_name ) ) ) {
		echo '<div class="error"><p>' . __( 'Whoops, that doesn\'t appear to be a sidebar that can be edited.', 'multiple-sidebar-generator' ) . '</p></div>';
			...
			<p>
				...
				$messages = array(
					1 => sprintf( '%s <a href="' . esc_url( admin_url( 'widgets.php' ) ) . '">%s</a>',
						__( 'Sidebar settings updated.', 'multiple-sidebar-generator' ),
						__( 'Update widgets now.', 'multiple-sidebar-generator' )
					)
				);
			...
			<div id="post-body" class="metabox-holder"><!--columns-2-->
				...
				<div id="post-body-content">
					<div class="sidebar-name-wrap">
						<label for="multiple-sidebar-generator-sidebar-name-new" class="screen-reader-text"><?php _e( 'Sidebar Name:', 'multiple-sidebar-generator' ); ?></label>
						<input type="text" name="simplepagesidebars_sidebar_name_new" id="multiple-sidebar-generator-sidebar-name-new" value="<?php echo esc_attr( $sidebar_name ); ?>" placeholder="<?php esc_attr_e( 'Enter sidebar name here', 'multiple-sidebar-generator' ); ?>" autocomplete="off">
					...
					<p class="submit">
						...
						?>
						<input type="submit" name="simplepagesidebars_sidebar_update" value="<?php esc_attr_e( 'Update Sidebar', 'multiple-sidebar-generator' ); ?>" class="button-primary">
